    ...
    <div class="chat-sidebar neumorphism-card"
      ...
      <div class="chat-type-tabs">
        ...
        >
          ...
          <span v-if="getChatTypeUnreadCount(chatTypes.GROUP) > 0" class="tab-unread-badge">
            {{ formatUnreadCount(getChatTypeUnreadCount(chatTypes.GROUP)) }}
          </span>
          <div class="tab-indicator"></div>
        </button>
...
const syncConversationUnreadCount = (conversationId, unreadCount) => {
  const normalizedCount = normalizeUnreadCount(unreadCount)
  const conversationLists = [friends.value, groups.value, aiChats.value, chatHistory.value]
