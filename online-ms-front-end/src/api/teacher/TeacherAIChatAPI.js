...
// 统一管理后端的API基地址
const API_BASE_URL = 'http://localhost:8088/api/teacher/ai';
 ...
 * AI小助手 - 课程简介
 * @param {object} params - 包含请求参数的对象
 * @param {string} params.question - 课程简介的初步内容。
 * @param {(data: object) => void} onData - 处理每个AIResponse对象的回调函数。对象结构为 { content: string, ... }
 * @param {() => void} onComplete - 流结束时的回调函数。
 * @param {(error: Error) => void} onError - 发生错误时的回调函数。
 */
export async function getOptimizationAssistantResponse(params, onData, onComplete, onError) {
    const { question } = params;
    const queryParams = new URLSearchParams({ question });

    try {
        // 使用教师端的通用请求函数
        const response = await apiFetch(`/optimizationAssistant?${queryParams.toString()}`);
        ...
        while (true) {
            ...
            while (buffer.includes('\n')) {
                ...
                if (line.startsWith('data:')) {
                    ...
                    if (content && onData) {
                        try {
                            // 后端流式返回AIResponse对象的JSON字符串
                            const aiResponse = JSON.parse(content);
                            onData(aiResponse);
                        } catch (e) {
                            console.error('解析AIResponse对象失败:', e, '原始数据:', content);
                        }
                    }
                }
            }
        ...
        console.error('课程简介优化请求失败:', error);
 ...
 * @param {(error: Error) => void} onError - 发生错误时的回调函数。
 */
export async function getOptimizationAssistantCrowd(params, onData, onComplete, onError) {
    const { question } = params;
    const queryParams = new URLSearchParams({ question });

    try {
        // 使用教师端的通用请求函数
        const response = await apiFetch(`/optimization/assistant/crowd?${queryParams.toString()}`);
 ...
 * @param {(error: Error) => void} onError - 发生错误时的回调函数。
 */
export async function getOptimizationAssistantRequirements(params, onData, onComplete, onError) {
    const { question } = params;
    const queryParams = new URLSearchParams({ question });

    try {
        // 使用教师端的通用请求函数
        const response = await apiFetch(`/optimization/requirements?${queryParams.toString()}`);
...
/**
 * 智能出题助手
 * @param {object} requestData - 包含生成题目所需参数的对象, 对应后端的 QuestionGenerationRequest
 * @returns {Promise<object|null>} - 成功时返回服务端的 Question 对象，失败时返回null。
 */
export async function getOptimizationAssistantQuestions(requestData) {
    if (!requestData) {
        console.error('请求失败：未提供请求参数');
        ElMessage.error('请求失败：参数缺失');
        return null;
    }

    try {
        const response = await apiFetch(`/optimization/assistant/questions`, {
            method: 'POST',
            headers: {
        ...
        // 后端直接返回Question对象，所以直接解析json
        return await response.json();

    } catch (error) {
        // apiFetch 内部已经处理了网络或认证等通用错误提示
        console.error('智能出题请求失败:', error);
        // 此处可以不重复显示ElMessage，因为apiFetch中已有
        return null;
    }
}