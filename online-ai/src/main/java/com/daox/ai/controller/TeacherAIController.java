...
import com.daox.ai.service.Impl.TeacherAIServerImpl;
...
public class TeacherAIController {
    ...
    @Resource
    private TeacherAIServerImpl teacherAIServerImpl;
    ...
    public Flux<AIResponse> streamAiChat(@RequestParam String question,
     ...
     * @return 优化后的课程简介
     */
    @GetMapping(value = "/optimizationAssistant", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<AIResponse> optimizationAssistant(String question) {
        log.info("optimizationAssistant: {}", question);
        return teacherAIServerImpl.optimizationAssistant(question).map(s -> AIResponse.builder().content(s).build());
    }
     ...
     * @return 适合人群
     */
    @GetMapping(value = "/optimization/assistant/crowd", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<AIResponse> optimizationAssistantCrowd(String question) {
        log.info("optimizationAssistantCrowd: {}", question);
        return teacherAIServerImpl.optimizationAssistantCrowd(question).map(s -> AIResponse.builder().content(s).build());
    }
     ...
     * @return “技术要求”清单
     */
    @GetMapping(value = "/optimization/requirements", produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<AIResponse> optimizationAssistantRequirements(String question) {
        log.info("optimizationAssistantRequirements: {}", question);
        return teacherAIServerImpl.optimizationAssistantRequirements(question).map(s -> AIResponse.builder().content(s).build());
    }
    ...
    public Question optimizationAssistantQuestions(@RequestBody QuestionGenerationRequest request) {
        // log.info("optimizationAssistantQuestions: {}", request);
        return teacherAIServerImpl.optimizationAssistantQuestions(request);
    }
