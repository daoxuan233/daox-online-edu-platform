...
public class ErrorPageController extends AbstractErrorController {
    ...
    public RestBean<Void> businessError(HttpServletRequest request) {
        BusinessException businessException = (BusinessException) request.getAttribute("exception");
        return RestBean.failure(Integer.parseInt(businessException.getCode()), businessException.getMessage());
    }
