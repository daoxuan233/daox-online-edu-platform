...
public class WebSocketSingleServiceConfig implements InitializingBean {
    ...
    public void onOpen(Session session) {
        ...
        if (userId != null) {
            numberOfConnectionsMap.put(userId, session);
    ...
    public void onClose(Session session) {
        ...
        if (userId != null) {
            numberOfConnectionsMap.remove(userId);
    ...
    public void onMessage(Session session, String message) {
        ...
        // 存储到数据库
        ChatMessage chatMessage = webSocketSingleService.addSingleMessage(bean);
        if (chatMessage != null) {
            // 将Java对象转换成为json字符串
