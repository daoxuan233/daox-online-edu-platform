...
public class JwtUtils {
    ...
    public UserDetails toUser(DecodedJWT jwt) {
        ...
        String[] authorities = claim.get("authorities").asArray(String.class);
        return User
                .withUsername(username) // 设置用户名
                .password("******")// 设置密码
                .authorities(authorities) // 设置用户的权限信息
                .build();
    }
     ...
     * @return JWT
     */
    public String createJwt(UserDetails userDetails, String username, String userId, String role, String identifier) {
        if (this.frequencyCheck(userId)) { // 频率检测
            Algorithm algorithm = Algorithm.HMAC256(key); // 使用 HMAC256 算法和密钥创建一个签名算法对象
            Date now = this.expireTime();
            String jwt = JWT.create()
                    .withJWTId(UUID.randomUUID().toString()) // 设置 JWT 的唯一标识符 -> 防止重复使用
                    .withClaim("id", userId) // 设置 JWT 的自定义声明，包含用户的 ID
                    .withClaim("username", username) // 用户的用户名
                    .withClaim("role", role) // 用户的角色信息
                    .withClaim("identifier", identifier) // 学号/工号
                    .withClaim("authorities", userDetails.getAuthorities()// 获取用户的权限信息
                            .stream() // 将权限信息转换为流
                            .map(GrantedAuthority::getAuthority).toList()) // 设置 JWT 的自定义声明，包含用户的权限信息
                    .withExpiresAt(now) // 设置 JWT 的过期时间
    ...
    public DecodedJWT resolveJWT(String headerToken) {
        ...
        if (parts.length != 3) {
            log.debug("收到了一个格式不正确的Token，已忽略");
            return null; // 格式不正确，直接返回null，不进行后续解析
        }
